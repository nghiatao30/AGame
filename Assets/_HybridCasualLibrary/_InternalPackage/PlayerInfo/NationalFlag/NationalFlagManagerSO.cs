...
[Serializable]
public class GeoPluginData
{
    public string geoplugin_request;
    public int geoplugin_status;
...
public class NationalFlagManagerSO : SerializedScriptableObject
{
    protected const string k_GeoPluginUrl = "http://geoplugin.net/json.gp";
    ...
    public class CountryInfo
    {
        ...
        public CountryInfo(Sprite nationalFlag)
        {
            m_NationalFlag = nationalFlag;
        }
        #endregion

        [SerializeField]
        private Sprite m_NationalFlag;

        public Sprite nationalFlag => m_NationalFlag;
    }

    [SerializeField]
    protected PPrefStringVariable m_PlayerCountryCode;
    [SerializeField]
    protected Dictionary<string, CountryInfo> m_CountryInfoDictionary;

    public virtual PPrefStringVariable playerCountryCode => m_PlayerCountryCode;

    protected virtual void OnEnable()
    {
        if (m_PlayerCountryCode.hasKey)
            return;
        GameManager.onSpawnCompleted += FetchCountryCode;
    }
    ...
    protected virtual void FetchCountryCode()
    {
        var coroutineRunner = CoroutineRunner.CreateCoroutineRunner(true);
        coroutineRunner.name = "CountryCodeDetection_CoroutineRunner";
        coroutineRunner.StartManagedCoroutine(FetchCountryCode_CR(coroutineRunner), CoroutineRunner.InteruptBehaviour.Replace);
    }

    /// <summary>
    /// Detect player country code -> send GET request to GeoPlugin server
    /// </summary>
    protected virtual IEnumerator FetchCountryCode_CR(CoroutineRunner coroutineRunner)
    {
        if (m_PlayerCountryCode.hasKey)
            yield break;
        bool isSuccess = false;
        var timeToWait = new WaitForSecondsRealtime(0.5f);
        do
        {
            using (var webRequest = UnityWebRequest.Get(k_GeoPluginUrl))
            {
                ...
                switch (webRequest.result)
                {
                    case UnityWebRequest.Result.Success:
                        GeoPluginData geoPluginData = null;
                        try
                        {
                            geoPluginData = JsonUtility.FromJson<GeoPluginData>(webRequest.downloadHandler.text);
                        }
                        catch (Exception exc)
                        {
                        ...
                        {
                            // Only set data only if http req status code is success
                            if (geoPluginData.geoplugin_status >= 200 && geoPluginData.geoplugin_status <= 299)
                            {
                                isSuccess = true;
                                m_PlayerCountryCode.value = geoPluginData.geoplugin_countryCode;
                            }
                        }
                        break;
                    default:
    ...
    public virtual bool ContainsCountry(string countryCode)
    {
        return m_CountryInfoDictionary.ContainsKey(countryCode);
    }

    public virtual CountryInfo GetCountryInfo(string countryCode)
    {
        return m_CountryInfoDictionary.Get(countryCode);
    }

    public virtual CountryInfo GetCountryInfoOfLocalPlayer()
    {
        return GetCountryInfo(m_PlayerCountryCode);
    }

    public virtual Sprite GetNationalFlag(string countryCode)
    {
        if (!ContainsCountry(countryCode))
        {
            // Return default US countrycode
            return GetCountryInfo("US").nationalFlag;
        }
        return GetCountryInfo(countryCode).nationalFlag;
    }

    public virtual KeyValuePair<string, CountryInfo> GetRandomCountryInfo()
    {
        var randomIndex = Random.Range(0, m_CountryInfoDictionary.Count);
        return m_CountryInfoDictionary.ElementAt(randomIndex);
    }
}