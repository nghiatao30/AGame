    ...
    static class ConditionalCompilationUtility
    {
        ...
        static ConditionalCompilationUtility()
        {
            ...
            AssemblyReloadEvents.afterAssemblyReload += () =>
            {
                if (!errorsFound)
                {
                    EditorPrefs.DeleteKey(k_PreviousUnsuccessfulDefines);
                    UpdateDependencies();
                }
            };
#else
        ...
        static void UpdateDependencies(bool reset = false)
        {
            ...
            //remove duplication
            HashSet<string> projectDefinesHashSet = new HashSet<string>();
            projectDefines.ForEach(define => projectDefinesHashSet.Add(define));
            projectDefines = new List<string>(projectDefinesHashSet);
            ...
            ForEachType(type =>
            {
                IEnumerable<Attribute> typeAttributes = null;
                try
                {
                    typeAttributes = type.GetCustomAttributes(false).Cast<Attribute>();   
                }
                catch (System.Exception) { 
                    //fail to load attributes 
                }
